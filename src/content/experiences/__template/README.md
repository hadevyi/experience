...
프로젝트의 `02 기술 스택`은 개발 기록으로서 의미가 크기 때문에, 가능한 한 언어/프레임워크/DB/IDE/도구의 상세 버전까지 확인해 적습니다.
단, README나 코드에서 확인되지 않은 버전은 임의로 추정하지 않고 빈 값으로 둡니다.
...
project:
  ...
  techStack:
    - category: language
      name: C#
      version: 가능한 한 실제 사용 버전까지 적습니다. 예) C# 6.0, Java 8, MySQL 5.7, Eclipse Oxygen
...
프로젝트에서는 `project.techStack.version`을 우선 채우는 것을 원칙으로 합니다.
버전을 확인할 수 없으면 임의로 추정하지 말고 비워둡니다.
